...
export const Navbar = () => {
  ...
  const topNotifRef = useRef(null);
  const subNotifRef = useRef(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      const isInsideTop = topNotifRef.current && topNotifRef.current.contains(event.target);
      const isInsideSub = subNotifRef.current && subNotifRef.current.contains(event.target);
      if (!isInsideTop && !isInsideSub) {
  ...
  const handleLogout = () => {
    ...
    showToast('Logged out of NEXORA');
    setCurrentPage('home');
    setIsDropdownOpen(false);
  };
  ...
  return (
    ...
    >
      {/* 1. Main Top Header Bar (Logo + Brand Name Left, Actions Right) */}
      <div 
        style={{ 
      ...
      >
        {/* FAR LEFT (START): Logo Image + Brand Name Side by Side */}
          ...
          >
            <img
              ...
              style={{ 
                ...
                filter: 'drop-shadow(0 2px 8px rgba(186, 12, 47, 0.25))' 
              }}
            />
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ 
                ...
                NEXORA<span style={{ color: '#ba0c2f' }}>.</span>
              </span>
              <span style={{ 
                fontSize: '0.55rem', 
                fontWeight: 800, 
        ...
        {/* CENTER (MIDDLE): Primary Navigation Links Bar */}
        <nav 
        ...
        >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? '#ba0c2f' : 'var(--text-main)',
                  borderRadius: '9999px',
                  border: 'none',
                  minHeight: '34px',
                  whiteSpace: 'nowrap',
          ...
          <div style={{ position: 'relative' }} ref={topNotifRef}>
            <button
              ...
              style={{
                ...
                color: isNotifOpen ? '#ba0c2f' : 'var(--text-main)',
                borderRadius: '9999px',
                border: 'none',
                minHeight: '34px',
                display: 'flex',
            ...
            >
              <Bell size={18} color={unreadCount > 0 || isNotifOpen ? '#ba0c2f' : 'currentColor'} />
              {unreadCount > 0 && (
                <span
                  style={{
                    background: '#ba0c2f',
            ...
            {/* Top Nav Bar Notifications Dropdown */}
            {isNotifOpen && (
              <div
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.6rem', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: '0.88rem', fontWeight: 800, color: 'var(--text-main)', whiteSpace: 'nowrap', flexShrink: 0 }}>Notifications</span>
                    {unreadCount > 0 && (
      ...
      {/* 2. Sub Header Row (Pages Navigation Bar Below Main Nav for Mobile) */}
      <div 
        className="sub-nav-links hide-desktop"
        style={{ 
          width: '100%',
          marginTop: '0.4rem',
          borderTop: '1px solid var(--border-light)',
          borderBottom: '1px solid rgba(186, 12, 47, 0.12)',
      ...
      >
        <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', maxWidth: 'var(--container-max)', width: '100%', whiteSpace: 'nowrap' }}>
          {navLinks.map((link) => {
            const targetPage = link.page || link.id;
            const isActive = currentPage === targetPage;
            return (
          ...
          {/* Nav Bar Notifications Pop Up Option */}
          <div style={{ position: 'relative' }} ref={subNotifRef}>
            <button
              onClick={() => setIsNotifOpen(!isNotifOpen)}
              className="btn"
              title="Notifications"
