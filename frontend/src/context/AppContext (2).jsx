...
const initialProducts = [
  ...
  {
    id: "p3",
    name: "NeoMatte Tech Hoodie",
    slug: "nexora-neomatte-tech-hoodie",
  ...
  {
    id: "p4",
    name: "CyberGlow Keyboard",
    slug: "nexora-cyberglow-keyboard",
...
export const AppProvider = ({ children }) => {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          return combined.map(p => {
            if (p.name) p.name = p.name.replace(/^(NEXORA|SWITCHES)\s+/i, '');
            if (p.category === 'Switches') p.category = 'Keyboards & Gear';
            if (!p.subCategory) {
  ...
  const [userAddresses, setUserAddresses] = useState(() => {
    try {
      const saved = localStorage.getItem('nexora_user_addresses');
      if (saved) {
        const parsed = JSON.parse(saved);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('nexora_user_addresses', JSON.stringify(userAddresses));
    } catch (e) {}
  }, [userAddresses]);
  ...
  const [vendors, setVendors] = useState(() => {
    try {
      const saved = localStorage.getItem('nexora_vendors');
      return saved ? JSON.parse(saved) : [
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Use promo code NEXORA20 at checkout to save 20% on all tech & apparel items.',
      time: '5m ago',
      unread: true,
      type: 'promo'
    },
  ...
  useEffect(() => {
    localStorage.setItem('nexora_vendors', JSON.stringify(vendors));
  }, [vendors]);
  ...
  const loginWithCredentials = async (email, password) => {
    ...
    if (cleanEmail === 'admin@aura.io' || cleanEmail === 'admin@nexora.io' || cleanEmail.includes('admin')) {
    ...
    } else if (cleanEmail === 'alex@aura.io' || cleanEmail === 'alex@nexora.io' || cleanEmail.includes('alex')) {
  ...
  const addProduct = (prodData) => {
    ...
    setProducts(prev => {
      ...
      localStorage.setItem('nexora_products', JSON.stringify(next));
      return next;
    });
  ...
  const applyCouponCode = (code) => {
    ...
    if (clean === 'NEXORA10' || clean === 'NEXORA10') {
      setAppliedCoupon({ code: 'NEXORA10', discountPercent: 10 });
      showToast('10% NEXORA Coupon Applied!');
      return true;
    } else if (clean === 'NEXORA20' || clean === 'NEXORA20') {
      setAppliedCoupon({ code: 'NEXORA20', discountPercent: 20 });
      showToast('20% NEXORA VIP Coupon Applied!');
      return true;
    } else {
  ...
  const placeOrder = async (shippingInfo, paymentMethod) => {
    ...
    showToast('NEXORA Order Placed & Saved to Database!');
    return createdOrder;
  };
  ...
  const [categoriesList, setCategoriesList] = useState(() => {
    try {
      const saved = localStorage.getItem('nexora_categories');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          return parsed.map(c => {
  ...
  const [banners, setBanners] = useState(() => {
    try {
      const saved = localStorage.getItem('nexora_banners');
      return saved ? JSON.parse(saved) : [
        {
          id: "b1",
  ...
  // Storage persistence effects
  useEffect(() => {
    localStorage.setItem('nexora_categories', JSON.stringify(categoriesList));
  }, [categoriesList]);
  ...
  useEffect(() => {
    localStorage.setItem('nexora_banners', JSON.stringify(banners));
  }, [banners]);
  ...
  const addCategory = (catData) => {
    ...
    setCategoriesList(prev => {
      ...
      localStorage.setItem('nexora_categories', JSON.stringify(next));
      return next;
    });
